...
public class FulfilledRequestsServletTest {

    @InjectMocks
    private FulfilledRequestsServlet fulfilledRequestsServlet;
    ...
    public void shouldDoGet() throws ServletException, IOException {
        ...
        when(requestService.getUserRequestsCount(RequestStatus.DONE.getName())).thenReturn(1);
        when(paginationDTO.getAmountOfItems()).thenReturn(1);
        when(requestService.getRequests(RequestStatus.DONE.getName()
                , paginationDTO)).thenReturn(Collections.singletonList(requestDTO));
        when(req.getRequestDispatcher("doneRequests.jsp")).thenReturn(requestDispatcher);

        fulfilledRequestsServlet.doGet(req, resp);
        ...
        verify(req).setAttribute("doneRequests", Collections.singletonList(requestDTO));
